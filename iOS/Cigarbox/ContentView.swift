...
struct ContentView: View {
    ...
    @State private var editingItem: CigarItem?
    ...
    @ViewBuilder
    private func row(for entry: CigarItem) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.name)
                .font(Theme.headlineFont)
                .foregroundColor(Theme.textPrimary)
            Text("Brand: \(entry.brand)  \u{2022}  Purchased: \(entry.purchaseDate)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
        }
        .padding(.vertical, 4)
...
struct ItemEditView: View {
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Brand (Padron)", text: $field1)
                        .focused($focusedField, equals: .field1)
                        .accessibilityIdentifier("field1Field")
                    TextField("Purchased (2025-11-02)", text: $field2)
            ...
            .navigationTitle(mode == .add ? "Add Cigar" : "Edit Cigar")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(name: name, brand: field1, purchaseDate: field2, notes: notes)
        case .edit(var entry):
            entry.name = name
            entry.brand = field1
            entry.purchaseDate = field2
            entry.notes = notes
            store.update(entry)
        }
        dismiss()
